(function(obj) {
		...
		return {
			...
			addFiles : function addFiles(files, oninit, onadd, onprogress, onend) {
				...
				if (zipWriter)
					...
					createZipWriter();
				} else if (creationMethod == "DB" ) {
                    writer = new zip.DBWriter();
                    createZipWriter();
                } else {
			...
			getBlobURL : function(callback) {
				zipWriter.close(function(blob) {
					var blobURL = creationMethod != "File" ? URL.createObjectURL(blob) : zipFileEntry.toURL();
					callback(blobURL);
					zipWriter = null;
				});
			},
	...
	(function() {
		...
		if (typeof requestFileSystem == "undefined")
			creationMethodInput.options.length = 2;
